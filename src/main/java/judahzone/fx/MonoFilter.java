package judahzone.fx;

import java.security.InvalidParameterException;

import judahzone.api.FX;
import judahzone.api.Frequency;
import judahzone.util.Constants;
import lombok.Getter;

/**	•  IIR Filter op ported from Gervill. MONO op (uses left channel only).
	•  Converted from FloatBuffer to float[] and adapted to FX API. */
public class MonoFilter implements FX {

	public enum Settings { Type, Frequency, Resonance }
	public static enum Type { HiCut, LoCut }

	private final int buf_size; // anti-alias upsample-able
	private final float IBS; // inverse buffer size
	private final int SR; // anti-alias upsample-able

	@Getter private final String name = MonoFilter.class.getSimpleName();
	@Getter private final int paramCount = Settings.values().length;
	@Getter private Type filterType;
	@Getter private float frequency = 666;
	private double resonancedB = 1;
	private final IIRFilter filter = new IIRFilter();


	public MonoFilter(Type type, float freq, float reso, int upsample) {
	    this(type, freq, upsample);
	    setResonance(reso);
	    filter.reset();
	}

	public MonoFilter(Type type, float freq, int upsample) {
	    buf_size = upsample * Constants.bufSize();
	    IBS = 1.0f / buf_size;
	    SR = upsample * Constants.sampleRate();
	    filterType = type;
	    setFrequency(freq);
	    filter.reset();
	}

	@Override public int get(int idx) {
	    if (idx == Settings.Type.ordinal())
	        return getFilterType().ordinal();
	    if (idx == Settings.Frequency.ordinal())
	        return frequencyToKnob(frequency);
	    if (idx == Settings.Resonance.ordinal())
	        return (int)(resonancedB * 4);
	    throw new InvalidParameterException();
	}

	@Override public void set(int idx, int value) {
	    if (idx == Settings.Type.ordinal())
	        setFilterType(Type.values()[value]);
	    else if (idx == Settings.Frequency.ordinal())
	        setFrequency(knobToFrequency(value));
	    else if (idx == Settings.Resonance.ordinal())
	        setResonance(value * 0.25f);
	    else throw new InvalidParameterException();
	}

	/**Set frequency of filter in Hz. Recommended range (20 - samplerate/2) */
	public void setFrequency(float hz) {
	    if (frequency == hz)
	        return;
	    frequency = hz;
	    filter.dirty = true;
	}

	/**Set resonance of filter in dB. Recommended range (0 - 30)*/
	public void setResonance(float db) {
	    if (resonancedB == db)
	        return;
	    db = Math.max(0, Math.min(30, db));
	    resonancedB = db;
	    filter.dirty = true;
	}

	/** @return resonance of filter. */
	public float getResonance() {
	    return (float) resonancedB;
	}

	/** Set filter type */
	public void setFilterType(Type filtertype) {
	    if (this.filterType == filtertype) return;
	    this.filterType = filtertype;
	    filter.dirty = true;
	}

	/** Process mono channel in-place */
	public void process(float[] mono) {
	    filter.filter2Replace(mono);
	}

	/** FX API: left is processed, right ignored (can be null) */
	@Override public void process(float[] left, float[] right) {
	    if (left != null) process(left);
	}

	class IIRFilter {

	    private boolean dirty = true;
	    private double x1;
	    private double x2;
	    private double y1;
	    private double y2;
	    private double a0;
	    private double a1;
	    private double a2;
	    private double b1;
	    private double b2;
	    private double gain = 1;
	    private double wet = 0;
	    private double last_wet = 0;
	    private double last_a0;
	    private double last_a1;
	    private double last_a2;
	    private double last_b1;
	    private double last_b2;
	    private double last_gain;
	    private boolean last_set = false;

	    void reset() {
	        dirty = true;
	        last_set = false;
	        wet = 0.0f;
	        gain = 1.0f;

	        x1 = 0; x2 = 0;
	        y1 = 0; y2 = 0;
	        a0 = 0; a1 = 0;
	        a2 = 0;
	        b1 = 0; b2 = 0;
	    }

	    private void calc() {

	    	if (!dirty)
	            return;

	        double rdB = resonancedB;

	        if (filterType == Type.HiCut) {
	            double r = (frequency / SR);
	            if (r > 0.45) {
	                if (wet == 0) {
	                    if (rdB < 0.00001) {
	                        wet = 0.0f;
	                    } else {
	                        wet = 1.0f;
	                    }
	                }
	                r = 0.45;
	            } else {
	                wet = 1.0f;
	            }

	            double c = 1.0 / (Math.tan(Math.PI * r));
	            double csq = c * c;
	            double resonance = Math.pow(10.0, -(rdB / 20));
	            double _q = Math.sqrt(2.0f) * resonance;
	            double _a0 = 1.0 / (1.0 + (_q * c) + (csq));
	            double _a1 = 2.0 * _a0;
	            double _a2 = _a0;
	            double _b1 = (2.0 * _a0) * (1.0 - csq);
	            double _b2 = _a0 * (1.0 - (_q * c) + csq);

	            this.a0 = _a0;
	            this.a1 = _a1;
	            this.a2 = _a2;
	            this.b1 = _b1;
	            this.b2 = _b2;

	        }

	        if (filterType == Type.LoCut) {
	            double r = (frequency / SR);
	            if (r > 0.45) {
	                r = 0.45;
	            }
	            if (r < 0.0001) {
	                r = 0.0001;
	            }
	            wet = 1.0f;
	            double c = (Math.tan(Math.PI * (r)));
	            double csq = c * c;
	            double resonance = Math.pow(10.0, -(rdB / 20));
	            double _q = Math.sqrt(2.0f) * resonance;
	            double _a0 = 1.0 / (1.0 + (_q * c) + (csq));
	            double _a1 = -2.0 * _a0;
	            double _a2 = _a0;
	            double _b1 = (2.0 * _a0) * (csq - 1.0);
	            double _b2 = _a0 * (1.0 - (_q * c) + csq);

	            this.a0 = _a0;
	            this.a1 = _a1;
	            this.a2 = _a2;
	            this.b1 = _b1;
	            this.b2 = _b2;

	        }
	        dirty = false;
	    }

	    private void filter2Replace(float[] data) {
	        if (data == null) return;
	        calc();
	        if (!last_set) {
	            last_a0 = a0;
	            last_a1 = a1;
	            last_a2 = a2;
	            last_b1 = b1;
	            last_b2 = b2;
	            last_gain = gain;
	            last_wet = wet;
	            last_set = true;
	        }

	        if (wet > 0 || last_wet > 0) {

	            double _a0 = this.last_a0;
	            double _a1 = this.last_a1;
	            double _a2 = this.last_a2;
	            double _b1 = this.last_b1;
	            double _b2 = this.last_b2;
	            double _gain = this.last_gain;
	            double _wet = this.last_wet;
	            double a0_delta = (this.a0 - this.last_a0) * IBS;
	            double a1_delta = (this.a1 - this.last_a1) * IBS;
	            double a2_delta = (this.a2 - this.last_a2) * IBS;
	            double b1_delta = (this.b1 - this.last_b1) * IBS;
	            double b2_delta = (this.b2 - this.last_b2) * IBS;
	            double gain_delta = (this.gain - this.last_gain) * IBS;
	            double wet_delta = (this.wet - this.last_wet) * IBS;
	            double _x1 = this.x1;
	            double _x2 = this.x2;
	            double _y1 = this.y1;
	            double _y2 = this.y2;

	            if (wet_delta != 0) {
	                for (int i = 0; i < buf_size; i++) {
	                    _a0 += a0_delta;
	                    _a1 += a1_delta;
	                    _a2 += a2_delta;
	                    _b1 += b1_delta;
	                    _b2 += b2_delta;
	                    _gain += gain_delta;
	                    _wet += wet_delta;
	                    double x = data[i];
	                    double y = (_a0 * x + _a1 * _x1 + _a2 * _x2 - _b1 * _y1 - _b2 * _y2);
	                    data[i] = (float) ((y * _gain) * _wet + (x) * (1 - _wet));
	                    _x2 = _x1;
	                    _x1 = x;
	                    _y2 = _y1;
	                    _y1 = y;
	                }
	            } else if (a0_delta == 0 && a1_delta == 0 && a2_delta == 0
	                    && b1_delta == 0 && b2_delta == 0) {
	                for (int i = 0; i < buf_size; i++) {
	                    double x = data[i];
	                    double y = (_a0 * x + _a1 * _x1 + _a2 * _x2 - _b1 * _y1 - _b2 * _y2);
	                    data[i] = (float) (y * _gain);
	                    _x2 = _x1;
	                    _x1 = x;
	                    _y2 = _y1;
	                    _y1 = y;
	                }
	            } else {
	                for (int i = 0; i < buf_size; i++) {
	                    _a0 += a0_delta;
	                    _a1 += a1_delta;
	                    _a2 += a2_delta;
	                    _b1 += b1_delta;
	                    _b2 += b2_delta;
	                    _gain += gain_delta;
	                    double x = data[i];
	                    double y = (_a0 * x + _a1 * _x1 + _a2 * _x2 - _b1 * _y1 - _b2 * _y2);
	                    data[i] = (float) (y * _gain);
	                    _x2 = _x1;
	                    _x1 = x;
	                    _y2 = _y1;
	                    _y1 = y;
	                }
	            }

	            if (Math.abs(_x1) < 1.0E-8)
	                _x1 = 0;

	            if (Math.abs(_x2) < 1.0E-8)
	                _x2 = 0;

	            if (Math.abs(_y1) < 1.0E-8)
	                _y1 = 0;

	            if (Math.abs(_y2) < 1.0E-8)
	                _y2 = 0;

	            this.x1 = _x1;
	            this.x2 = _x2;
	            this.y1 = _y1;
	            this.y2 = _y2;
	        }

	        this.last_a0 = this.a0;
	        this.last_a1 = this.a1;
	        this.last_a2 = this.a2;
	        this.last_b1 = this.b1;
	        this.last_b2 = this.b2;
	        this.last_gain = this.gain;
	        this.last_wet = this.wet;

	    }

	}

public static float knobToFrequency(int val) {
    // Map knob (0..99) to percent (1..100) and use logarithmic mapping
    int percent = Math.max(1, Math.min(100, val + 1));
    return Constants.logarithmic(percent, Frequency.MIN, Frequency.MAX);
}

public static int frequencyToKnob(float freq) {
    // Use reverseLog to map frequency back to percent (1..100), then to 0-based knob index
    return Constants.reverseLog(freq, Frequency.MIN, Frequency.MAX);
}



}